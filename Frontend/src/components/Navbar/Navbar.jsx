...
const Navbar = () => {
  ...
  //COMBINE UPDATING LOGIN MODEL NAD AUTH STATUS ON LOCATION CHANGES
  ...
  //   if (location.pathname === "/login" && localStorage.getItem("loginData")) {
  //     navigate("/");
  //   }
  ...
  useEffect(() => {
    // Check if we are already logged in while trying to access /login
    const loggedIn = !!localStorage.getItem("loginData");

    // Sync the state in case it changed elsewhere
  ...
  const renderMobileAuthButton = () => {
    return isAuthenticated ? (
      <button
        onClick={handleLogout}
        className="w-full px-4 py-3 from-amber-500 to-amber-700 text-[#2D1B0E] rounded-xl font-semibold border-2 border-amber-900/30 hover:border-amber-600/50 items-center justify-center space-x-2 text-sm "
      >
        <FiLogOut />
        <span>Logout</span>
      </button>
